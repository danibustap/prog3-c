def statistik(dateiname):
    ...
    with open(dateiname) as file: #with closes the file when the block finishes
        for line in file:
            ...
            #data = { "65197": { "Meier": 42}}
    for plz in sorted(data): #sorted ordenada la lista por PLZ en orden ascendente
        customers = [f"{name}({total})" for name, total in sorted(data[plz].items())] 
        #list comprehension / items devuelve una tupla de pares (key, value)/sorted(data[plz]) ordena alfabeticamnete por name
